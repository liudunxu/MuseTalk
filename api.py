...
# Segment-consistency primitives (extracted from the
# ``MuseTalkApiRuntime`` class so they can be unit-tested
# without importing this module's heavy ML deps).
from musetalk.utils.segment_consistency import (
...
class LipSyncRequest(BaseModel):
    ...
    codeformer_fidelity_weight: float = Field(
        ...
        description="CodeFormer fidelity weight. 0 = sharpest (most codebook-driven, identity drift), "
                    "1 = closest to input. Default 0.85 keeps the restored face close to the "
                    ...
                    "codeformer_temporal_alpha to tame the flicker. Higher (0.9-0.95) almost "
    ...
    codeformer_temporal_alpha: float = Field(
        ...
        description="Cross-frame EMA on the CodeFormer-restored face crops. After the model "
                    ...
                    "CodeFormer output, may flicker). 0.6-0.8 dampens the per-frame "
                    "variation that causes flicker without lagging lipsync too much. 0 disables "
                    "the EMA.",
    )
...
class MuseTalkApiRuntime:
    ...
    def _write_result_frames(
        ...
        if segment_consistency_enforced:
            enforced, seg_reasons = enforce_segment_consistency(
        ...
        with self.run_lock:
            ...
            if payload.codeformer_enabled and generated:
                ...
                if restorer is not None and restorer.is_loaded:
                    ...
                    # lipsync motion too much. Only mix adjacent
